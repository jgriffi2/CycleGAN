...
def discriminator(input, options, reuse=False, name='discriminator'):
    with tf.variable_scope(name):
        ...
        c1 = lrelu(conv2d(input, options.df_dim, ks=4, s=2, name='c1'), name='lr1')
        # c1: H/2 x W/2 x options.df_dim
        c2 = lrelu(instance_norm(conv2d(c1, options.df_dim*2, ks=4, s=2, name='c2'), name='i2'), name='lr2')
        # c2: H/4 x W/4 x options.df_dim*2
        c3 = lrelu(instance_norm(conv2d(c2, options.df_dim*4, ks=4, s=2, name='c3'), name='i3'), name='lr3')
        # c3: H/8 x W/8 x options.df_dim*4
        c4 = lrelu(instance_norm(conv2d(c3, options.df_dim*8, ks=4, s=1, name='c4'), name='i4'), name='lr4')
        # c4: H/8 x W/8 x options.df_dim*8
        c5 = conv2d(c4, 1, ks=4, s=1, name='c5')
        # c5: H/8 x W/8 x 1
...
def generator(input, options, reuse=False, name='generator'):
    def res_block(input, nk, ks=3, s=1, name='res_block'):
        c1 = relu(instance_norm(conv2d(input, nk, ks, s, name=name+'_c1'), name=name+'_i1'), name=name+'_r1')
    ...
    with tf.variable_scope(name):
        ...
        c1 = relu(instance_norm(conv2d(input, options.gf_dim, ks=7, s=1, name='c1'), name='i1'), name='r1')
        ...
        c3 = relu(instance_norm(conv2d(c2, options.gf_dim*4, ks=3, s=2, name='c3'), name='i3'), name='r3')
        ...
        c4 = tanh(conv2d(t2, options.output_nc, ks=7, s=1, name='c4'), name='tan1')
